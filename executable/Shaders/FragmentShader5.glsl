...
float calculateShadow(vec4 fragment_position_light);
...
vec3 calculateDirectionalLight(Directional_Light light, vec3 normal, vec3 view_direction)
{
    ...
    float shadow = calculateShadow(fs_in.fragment_position_light);
...
float calculateShadow(vec4 fragment_position_light)
{
  vec3 projection_coordinates = fragment_position_light.xyz/fragment_position_light.w;
